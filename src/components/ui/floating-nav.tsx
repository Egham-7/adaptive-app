  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "hidden lg:flex max-w-fit fixed top-6 inset-x-0 mx-auto border border-border/40 rounded-full bg-background/80 backdrop-blur-md shadow-lg z-[5000] px-6 py-3 items-center justify-center gap-6",
          className
        )}
      >
        {/* Navigation Items */}
        {navItems.map((navItem, idx: number) => {
          ...
          return (
            <Link
              key={`link=${idx}`}
              href={navItem.link}
              className="relative text-muted-foreground hover:text-foreground transition-colors duration-200 flex items-center gap-1 px-3 py-2 rounded-full"
            >
              {isActive && (
                <motion.div
                  layoutId="floating-nav-lamp"
