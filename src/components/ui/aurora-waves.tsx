...
// Emerald green palette (matching gradient button theme)
vec3 palette(float t) {
    // Convert emerald colors to vec3 (RGB normalized to 0-1)
    vec3 lime = vec3(0.639, 0.902, 0.208);      // #a3e635
    vec3 emerald = vec3(0.063, 0.725, 0.506);   // #10b981
    vec3 deepEmerald = vec3(0.020, 0.400, 0.412); // #059669

    // Create a smooth gradient between emerald colors
    float phase = 0.5 + 0.5 * sin(t * 3.14159);
    vec3 col1 = mix(deepEmerald, emerald, phase);
    vec3 col2 = mix(emerald, lime, phase);

    return mix(col1, col2, 0.5 + 0.5 * sin(t * 2.0));
}
